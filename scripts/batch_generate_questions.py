...
生成規則：
- 專利法（patent-act）：100題雙語配對（中文題目 + 對應的英文題目）
- 訴願法（administrative-appeal）：200題中文
- 行政訴訟法（administrative-litigation）：200題中文
- 審查基準（examination-guidelines）：200題中文

題型分配：50% MCQ + 50% ShortAnswer
"""
import os
import sys
import logging
...
def generate_bilingual_questions_for_patent_act(generator: QuestionGenerator, count: int = 100):
    """
    為專利法生成雙語配對題目
    
    Args:
        generator: QuestionGenerator 實例
        count: 要生成的題目對數（預設 100 對）
    ...
    for article_num in sorted(common_articles, key=lambda x: int(x)):
        ...
        try:
            logger.info(f"生成第 {article_num} 條的 {question_type} 題目 ({to_generate} 對)")
            ...
            for zh_q, en_q in question_pairs:
                ...
                en_q["lang"] = "en"
                en_q["paired_question_id"] = str(result_zh.inserted_id)  # 記錄配對關係
...
def main():
    ...
    # 1. 生成專利法雙語題目（100對 = 200題）
    logger.info("\n" + "=" * 60)
    logger.info("任務 1/4：專利法雙語題目")
    logger.info("=" * 60)
    pairs = generate_bilingual_questions_for_patent_act(generator, count=100)
    results["patent-act"]["zh-TW"] = pairs
    results["patent-act"]["en"] = pairs
    
    # 2. 生成訴願法題目（200題中文）
    logger.info("\n" + "=" * 60)
    logger.info("任務 2/4：訴願法中文題目")
    logger.info("=" * 60)
    ...
    # 4. 生成審查基準題目（200題中文）
    logger.info("\n" + "=" * 60)
    logger.info("任務 4/4：審查基準中文題目")
    logger.info("=" * 60)
    
    # 檢查是否有審查基準資料
    from db.models import laws_collection as laws_coll
    exam_count = laws_coll.count_documents({"type": "examination-guidelines"})
    if exam_count == 0:
    ...
    logger.info(f"總計：{sum([results[k].get('zh-TW', 0) + results[k].get('en', 0) for k in results])} 題")
